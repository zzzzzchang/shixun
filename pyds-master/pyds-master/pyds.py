...
class Evidence(object): # 证据

    def __init__(self):  # 将evidence初始化为空
        self.evidence = []

    def setEvidence(self, BPA, focalElemnt):  # 设置证据，每组证据由若干BOE组成，BOE[m(A), A]， Evidence = [BOE1, BOE2,...]
        return self.evidence.append([BPA, focalElemnt])

    def getEvidence(self):  # 获取证据
        return self.evidence


    def getBOECardinality(self, n):  # 获取第n个BOE的基数|A|
        return len(self.getEvidence()[n][1])

    def getMass(self, n):  # 获取第n个BOE的mass
        return self.getEvidence()[n][0]

    def calculate_pl(self, FOD):  # 基于当前证据，计算识别框架内，每个元素的似然函数
        pl = []
        fod = list(FOD)  # 传入集合形式的识别框架FOD，将它转化为列表形式
        for j in range(len(fod)):  # 循环，对FOD中每个元素进行计算
            pl_value = 0
            for k in range(len(self.evidence)): # 基于当前证据组
                if set({fod[j]}) & self.evidence[k][1] != set():
                    pl_value = pl_value + self.evidence[k][0]
            pl.append([fod[j], pl_value])
        return pl



class DS(object):  # DS证据组，拥有一个FOD和多个证据组

    def __init__(self):  # 将证据组和FOD都初始化为空
        self.DS = []
        self.FOD = set()
        self.pl_result = []
    ...
    def getFODCardinality(self):  # 获取FOD的基数|X|
        return len(self.FOD)

    def getEvidenceNum(self, n):  # 获取第n个witness拥有多少个mass
        return len(self.DS[n][0].getEvidence())

    def pl(self):  # 计算证据组中，识别框架内元素在各组证据中的pl值
        for j in range(len(self.getDS())):  # len(ds.getDS())为证据组数
            self.pl_result.append(self.getDS()[j][0].calculate_pl(self.FOD))
        return self.pl_result



# Deng 熵
def deng_Ent(ds):
    countWitness = len(ds.getDS())
    ent = np.zeros(countWitness, dtype=float)
    for j in range(countWitness):
        for k in range(ds.getEvidenceNum(j)):
            m = ds.getDS()[j][0].getMass(k)  # mass
            a = ds.getDS()[j][0].getBOECardinality(k) # BOE基数
            if m != 0:
                ent[j] = ent[j] - m * math.log(m / (2**a - 1), 2)
...
def improved_Belief_Ent(ds):
    ...
    for j in range(countWitness):
        for k in range(ds.getEvidenceNum(j)):
            m = ds.getDS()[j][0].getMass(k)  # mass
            a = ds.getDS()[j][0].getBOECardinality(k)  # BOE基数
            x = ds.getFODCardinality()  # FOD基数
            if m != 0:
                ent[j] = ent[j] - m * math.log((m / (2 ** a - 1)) * (a / x), 2)
    #print(ent)
    return ent


# 文献3
def weighted_Belief_Ent(ds):
    countWitness = len(ds.getDS())
    ent = np.zeros(countWitness, dtype=float)
    for j in range(countWitness):
        for k in range(ds.getEvidenceNum(j)):
            ...
            x = ds.getFODCardinality()  # FOD基数
            if m != 0:
                ent[j] = ent[j] - m * (a / x) * math.log(m / (2 ** a - 1), 2)
    #print(ent)
    return ent


# 文献4
def proposed_Belief_Ent(ds, s):
    countWitness = len(ds.getDS())
    ent = np.zeros(countWitness, dtype=float)
    for j in range(countWitness):
        for k in range(ds.getEvidenceNum(j)):
            ...
            x = ds.getFODCardinality()  # FOD基数
            if m != 0:
                ent[j] = ent[j] - m * math.log((m / (2 ** a - 1)) * s**((a - 1) / x), 2)
    #print(ent)
    return ent


# 文献9
def pan_Proposed_Belief_Ent(ds):
    countWitness = len(ds.getDS())
    ds.pl()  # 计算出pl值
    pt = []  # 初始化pt列表 pt = pl / sum(pl)
    for j in range(len(ds.pl_result)):
        s = 0
        temp = []
        for k in range(len(ds.pl_result[j])):
            s = s + ds.pl_result[j][k][1]
        for k in range(len(ds.pl_result[j])):
            if s != 0:
                t = ds.pl_result[j][k][1] / s
            temp.append([ds.pl_result[j][k][0], t])
        pt.append(temp)
    ent = np.zeros(countWitness, dtype=float)
    # print(countWitness)
    for j in range(countWitness):  # 在第j个witness中
        for k in range(ds.getEvidenceNum(j)):  # 在第k个BOE中
            m = ds.getDS()[j][0].getMass(k)  # mass
            a = ds.getDS()[j][0].getBOECardinality(k)  # BOE基数
            s = ds.getDS()[j][0].getEvidence()[k][1]
            pm = 0
            for b in range(len(pt[j])):
                if set({pt[j][b][0]}).issubset(s):
                    pm = pm + pt[j][b][1]
            if pm != 0:
                ent[j] = ent[j] + m * math.log(a / pm, 2)
    #print(ent)
    return ent
...
# 文献10
def improved_Deng_Ent(ds):
    countWitness = len(ds.getDS())
    ent = np.zeros(countWitness, dtype=float)
    for j in range(countWitness):  # 第j个witness
        for k in range(ds.getEvidenceNum(j)):  # 第j个witness中第k个证据
            m = ds.getDS()[j][0].getMass(k)  # mass
            a = ds.getDS()[j][0].getBOECardinality(k)  # BOE基数
            x = ds.getFODCardinality()  # FOD基数
            p = 0
            for c in range(ds.getEvidenceNum(j)):
                if ds.getDS()[j][0].getEvidence()[k][1] != ds.getDS()[j][0].getEvidence()[c][1]:  # B != A
                    p = p + (len(ds.getDS()[j][0].getEvidence()[k][1] & ds.getDS()[j][0].getEvidence()[c][1]) / (2 ** x - 1))
            if m != 0:
                ent[j] = ent[j] - m * math.log((m / (2 ** a - 1)) * math.exp(p), 2)
    #print(ent)
    return ent
